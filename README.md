...
`docs/video.html` 是上傳影片版：選擇影片檔（mp4/mov/mp3/wav…）後，
在瀏覽器內用 [Whisper](https://github.com/openai/whisper) 語音辨識模型
（透過 transformers.js，純前端執行）轉出**帶時間戳的口白逐字稿**，
再套用同一套規則引擎，輸出：

- 整體判定與風險分數（口白＋文案合併評估）
- **需優化的字詞清單（依時間排序）**：`00:06–00:14 命中「七天瘦五公斤」→ 建議…`，
  點時間戳可直接跳轉影片該秒數播放
- 完整逐字稿（命中段落黃底標示）
- 發佈前人工檢查清單
...
- 影片**不會上傳到任何伺服器**，辨識在使用者的瀏覽器本地執行；
  第一次使用需下載辨識模型（tiny 約 40MB／base 約 80MB／small 約 250MB，之後快取）
- 語音辨識輸出多為簡體中文，`detector.js` 內建簡→繁正規化後再比對規則
- 辨識會有錯字與時間誤差，模型越大越準；長影片辨識時間依裝置效能而定
- 需要較新的瀏覽器（建議桌面版 Chrome/Edge）
